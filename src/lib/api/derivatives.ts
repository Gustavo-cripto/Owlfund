// Derivados da OKX (open interest, long/short, funding, CVD, taker, velas,
// put/call) + um score de sentimento composto. Vive numa biblioteca para servir
// os dois consumidores: a rota /api/derivatives (site) e o MCP (agentes).
// Dados públicos de mercado; a rota é que trata da sessão e do limite.
...
/** Painel de derivados de um símbolo (ex.: BTC, ETH). */
export async function getDerivatives(symbol: string) {
  const base = (symbol || "BTC")
  ...
  return { symbol: base, oi, longShort, funding, cvd, taker, candles, putCall, score, rsi: rsiVal, components, missing: hasPutCall ? [] : ["putCall"] };
}
